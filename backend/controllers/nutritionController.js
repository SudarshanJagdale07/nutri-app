...
// instantiate client using sanitized URI to avoid parse errors for unsupported options
const client = new MongoClient(MONGO_URI);
await client.connect();
const db = client.db("nutrition_ai_projectDB");

// Collections used by the controller
const nutritionItems = db.collection("food_nutrition_DB"); // curated nutrition DB
const mealsCol = db.collection("meals");                   // store meals here
const dailyNutritionCol = db.collection("daily_nutrition"); // canonical per-user/day totals
const estimatedCol = db.collection("llm_estimated_foods"); // unknown foods for enrichment
...
// - Inserts meal into meals collection and upserts daily_nutrition (only when persist !== false)
...
export async function logFoodText(req, res) {
  try {
    // Support both body forms: { text, userId, persist } OR payload-style from frontend wrapper
    const body = req.body || {};
    ...
    // Make persist handling explicit: frontend may pass persist:false for analysis-only
...
- Where possible, detect the user's intent about where the food came from (home / outside / packaged) and return it as either item.preparation or top-level preparationHint.
...
`.trim();
    ...
    for (let itemIndex = 0; itemIndex < parsed.items.length; itemIndex++) {
      ...
      const isPiece = unit === "piece" || unit === "serving" || (foodDoc.perQuantity === 1);

      let calories, protein, carbs, fat, fiber, sugar;

      if (isPiece) {
        // Piece-based: use stored values as per-piece, multiply by quantity
        const qty = item.quantity;
        calories = (foodDoc.caloriesPer100g ?? foodDoc.calories_kcal ?? 0) * qty;
        protein = (foodDoc.proteinPer100g ?? foodDoc.protein_g ?? 0) * qty;
        carbs = (foodDoc.carbsPer100g ?? foodDoc.carbs_g ?? 0) * qty;
        fat = (foodDoc.fatPer100g ?? foodDoc.fat_g ?? 0) * qty;
        fiber = (foodDoc.fiberPer100g ?? foodDoc.fiber_g ?? 0) * qty;
        sugar = (foodDoc.sugarPer100g ?? foodDoc.sugar_g ?? 0) * qty;
      } else {
    ...
    const shouldPersist = persistFlag !== false; // default true, frontend sends false for analysis
...
export async function logImageMeal(req, res) {
  try {
    ...
    const insertRes = await mealsCol.insertOne(mealDoc);
    const savedMeal = { ...mealDoc, _id: insertRes.insertedId };

    // Upsert daily totals
    const dailyFilter = { userId, date };
    const dailyUpdate = {
      $inc: {
        completedCalories: Number(mealDoc.totalCalories || 0),
        completedProtein: Number(mealDoc.totalProtein || 0),
        completedCarbs: Number(mealDoc.totalCarbs || 0),
        completedFats: Number(mealDoc.totalFats || 0),
        completedFiber: Number(mealDoc.totalFiber || 0),
        completedSugar: Number(mealDoc.totalSugar || 0)
      },
      $addToSet: { mealIds: insertRes.insertedId },
      $setOnInsert: { userId, date, createdAt: timestamp }
    };
    await dailyNutritionCol.updateOne(dailyFilter, dailyUpdate, { upsert: true });
