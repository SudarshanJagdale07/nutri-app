    ...
    <div className="min-h-screen bg-[#F2F6F2] text-[#1A202C] relative overflow-hidden pb-24 font-sans">
      ...
      <style>{`
        ...
        .animate-scan {
          ...
          /* keep pointer-events none so it doesn't block interactions */
          pointer-events: none;
        }
...
const ProgressRing = ({ title, consumed, goal, percent, remaining, unit, hasGoal }) => {
  ...
  useEffect(() => {
    // If there's no goal, animate to full circumference (so it visually looks empty)
  ...
  // Helper to format: shows 1 decimal if needed, hides .0
  const formatNum = (num) => {
    return Math.round(Number(num) * 10) / 10;
  };

  // Display logic per user's request
  const insideText = () => {
    if (!hasGoal) return "-%";
    // has goal: show percent (0-100)
    return `${formatNum(percent)}%`;
  };

  const belowText = () => {
    if (!hasGoal) return "-/-";
    // has goal: show consumed / goal (both numbers)
